...
export function Dashboard({ currentUser, onViewChange }: DashboardProps) {
  ...
  const processStats = {
    ...
    inProgress: activeProcesses.filter(p => p.state === 'IN_PROGRESS').length,
    pending: activeProcesses.filter(p => p.state === 'PENDING_APPROVAL').length,
    draft: activeProcesses.filter(p => p.state === 'DRAFT').length,
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1>Bienvenido, {currentUser.full_name.split(' ')[0]}</h1>
          <p className="text-muted-foreground">
            Dashboard - Sistema de Gestión Documental FIEC
          </p>
        </div>
        <Button onClick={() => onViewChange('processes', { action: 'new' })}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm">Procesos Activos</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl">{processStats.inProgress}</div>
            <p className="text-xs text-muted-foreground mt-1">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl">{processStats.pending}</div>
            <p className="text-xs text-muted-foreground mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={({ name, percent }) => `${statusLabels[name] || name}: ${(percent * 100).toFixed(0)}%`}
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="value"
                >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Procesos que requieren tu atención</CardDescription>
          </CardHeader>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">{process.title || processType?.name}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Calendar className="w-3 h-3" />
                        <span>{process.year} - Mes {process.month}</span>
                      </div>
                    </div>
