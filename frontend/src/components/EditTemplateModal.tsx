...
interface TemplateStep {
    ...
    originalId?: number; // Real ID from backend if exists
    title: string;
    description: string;
    required: boolean;
    ord: number;
...
export function EditTemplateModal({ open, onClose, templateId, onTemplateUpdated, onTemplateDeleted }: EditTemplateModalProps) {
    ...
    const handleUpdate = async () => {
        ...
        // The update DTO might behave differently in NestJS depending on how it's implemented (replace vs merge).
        // Assuming the backend handles "replacing steps" if we send them, or we might need a specific logic.
        // For now, let's assume we send the full object to update.

        // NOTE: Backend update logic often requires specific handling for relations (steps). 
        // If the backend simply updates scalar fields, steps won't change.
        // Use `createProcessTemplate` style payload but for update.
